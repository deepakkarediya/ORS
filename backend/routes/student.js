...
router.get("/fetchstudent", fetchregis, async (req, res) => {
  try {
    ...
    const perPage=2;
    const totalPosts=await Student.countDocuments();    
    ...
    const student = await Student.find({ regis: req.regis1.id })
      ...
      .limit(perPage)
      .exec();     
      res.status(200).json({student,page,totalPosts,totalPages});
...
router.post(
  ...
  [
    ...
    body("mobileNo", "mobileno must be atleast 10 char...").isLength({
      min: 10,
    }),
